...
const CryptographicTextArea = ({mode, value, lastUsedValue, handleChange, maxLength = 5000, active = true}) => {
    ...
    return (
        <Form.Group as={Container} className="pe-0 ps-0">
            <Form.Label>
                {`${mode == "plaintext" ? "Plaintext" : "Ciphertext"} ${value.length != 0 && active ? `(${maxLength - value.length} characters left)` : ""}`}
            </Form.Label>
            <Row>
                ...
                onChange={(e) => {
                    ...
                    if (mode == "plaintext") newValue = newValue.toLowerCase();
            ...
            <Row className="mt-1">
                ...
                {lastUsedValue != "" && lastUsedValue != value ? `The ${mode == "plaintext" ? "plaintext" : "ciphertext"} has changed since last operation` : ""}
                </Col>
